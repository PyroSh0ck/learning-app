...
export default function Root({
  ...
  setGuides_t: Dispatch<SetStateAction<StudyGuide_t[]>>;
  ...
  useEffect(() => {
    if (input1.trim() !== "" || input2.trim() !== "" || input3.trim() !== "#000000" || input4.trim() !== "" || input5.trim() !== "") {
  ...
  const deleteTagHandler = async () => {
    try {
      ...
      setSelectedTags((prev) =>
        prev.filter((tag) => tag.id !== currentTag!.id)
      );
      resetTagEditor();
      setTagEditOpen(false);
      ...
      console.error(
        "An unexpected error has occurred with deleting data: ",
        err
      );
    }
  };
  ...
  const submitHandler = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (input1.trim() === "") {
      ...
      if (!res.ok) {
        ...
        console.error("Error creating study guide:", errorData.message);
  ...
  return (
    <div
      className={`root-holder outline-4 z-20 outline-offset-2 outline-purple-600 opacity-99 shadow-xl/30 absolute top-1/4 rounded-3xl bg-white ${tagEditOpen ? "w-1/2 left-1/4" : "w-1/3 left-1/3"} h-1/2 flex-row justify-center items-center ${modalOpen ? "flex" : "hidden"}`}
    >
      <div
        className={`relative modal-wrapper flex flex-col justify-start items-start w-9/10 h-9/10 pr-4 border-purple-600 ${tagEditOpen ? "border-r-4" : "border-r-0"} `}
      >
        <h1 className="p-5 font-black text-purple-700 text-2xl self-center mt-5">
          Create Study Guide
        </h1>
        <hr
          className={`${tagEditOpen ? "w-4/5" : "w-full"} self-center h-1 rounded-lg bg-purple-700 mb-5`}
        />
        <div
        ...
        >
          ...
          <div className="search-input-wrapper relative w-full flex flex-row justify-left items-center m-5">
            <h1 className="input-label text-purple-700 font-bold text-lg text-nowrap">
              Tags:
            </h1>
            <div className="relative w-3/8 ml-3">
              ...
              />
              <div
                className={`dropdown-wrapper w-full ring-2 z-20 ring-inset ring-purple-600 overflow-y-scroll no-scrollbar flex-col bg-white mt-1 rounded-lg min-h-40 justify-start items-center absolute top-full right-0 ${dropdownOpen ? "flex" : "hidden"}`}
              >
                <div
                  className={`flex flex-col items-center justify-start bg-white w-full rounded-lg border-purple-600 border-2 min-h-40`}
                >
                  {input2.trim() !== "" && !tagList.includes(input2.trim()) ? (
                    <>
                      <div
                        className="createTag cursor-pointer rounded-lg sticky top-0 z-10 p-2 text-purple-700 w-full text-center hover:bg-gray-100"
                        onClick={() => {
                          setTagEditOpen(true);
                          setInput3("#000000");
                        }}
                  ...
                  {dropdownOptions.map((tag) => (
                    <>
                      <div
                        key={tag.id}
                        className="dropdownTag cursor-pointer w-full text-center hover:bg-gray-200"
                        onClick={() => {
          ...
          <div className="selected-tags-wrapper flex flex-row justify-start px-5 items-center w-full">
            {selectedTags.map((tag) => (
              <div
                ...
                className="selectedTag relative bg-purple-800 m-2 px-5 py-2 rounded-md text-white flex flex-row justify-center items-center space-evenly"
              >
                <h1
                  className="selectedTagText font-extrabold cursor-pointer"
                  onClick={() => {
                    setCurrentTag(tag);
                    setInput2(tag.name);
                ...
                <div
                  className="selectedTagX cursor-pointer hover:text-[.7rem] transition-all duration-175 ease-in-out absolute text-[.6rem] top-0 right-0 font-black m-1 mr-1.5"
                  onClick={() => {
                    setSelectedTags((prevItems) =>
                      prevItems.filter((item) => item.name !== tag.name)
                    );
                  }}
                >
                  X
          ...
          <div className="buttons-wrapper flex flex-row justify-end w-full items-center">
            <button
              type="submit"
              className="bg-purple-700 p-4 rounded-xl hover:bg-purple-800 text-white transition-colors duration-175 ease-in-out font-black cursor-pointer m-5"
            >
              Create Study Guide
            </button>
          </div>
      ...
      <div
        className={`relative pl-4 editor-wrapper min-w-2/5 h-full flex-col justify-start items-start ${tagEditOpen ? "flex" : "hidden"}`}
      >
        <div
          className="x-button m-4 mr-6 cursor-pointer hover:text-2xl transition-all duration-175 ease-in-out absolute self-end top-0 font-black text-xl text-purple-700"
          onClick={() => {
            setInput2("");
            resetTagEditor();
            setTagEditOpen(false);
        ...
        <h1 className="text-xl font-black text-purple-600 mt-20 mb-10">
          Create or Update Tag
        </h1>
        <div className="tagName font-bold text-purple-600">
          Tag Name: <span className="text-black">{input2.trim()}</span>
        </div>
        {!tagList.includes(input2.trim()) ? (
          <>
            <div className="colorPicker-wrapper flex flex-row justify-start items-center">
              <h1 className="font-bold text-purple-600 text-nowrap">
                Choose a Color:{" "}
              </h1>
              <input
                value={input3}
            ...
            <button
              className="text-lg absolute right-0 bottom-0 bg-purple-700 p-2 rounded-xl hover:bg-purple-800 text-white transition-colors duration-175 ease-in-out font-black cursor-pointer m-5"
          ...
          <>
            <div className="relative colorPicker-wrapper flex flex-row justify-start items-center mb-8 mt-8">
              <input
                type="checkbox"
                checked={!tagEditOpen ? false : !nameEnabled}
                className="peer mr-2"
                onChange={() => {
                  setInput4("");
              ...
              />
              <h1 className="peer-checked:text-gray-500 font-bold text-purple-600 text-nowrap">
                Update Tag Name:{" "}
              </h1>
              <input
                value={input4}
              ...
              />
              {nameErr && (
                <p className="absolute top-full text-red-500 text-xs mt-1 ml-5">
                  Please enter a new and valid name or deselect the field.
                </p>
              )}
            </div>
            <div className="relative colorPicker-wrapper flex flex-row justify-start items-center">
              <input
                type="checkbox"
                checked={!tagEditOpen ? false : !colorEnabled}
                className="peer mr-2"
                onChange={() => {
                  setInput3(
                    dropdownOptions.find((value) => value.name === input2)
                      ?.color ?? "#000000"
                  );
                  setColorEnabled(!colorEnabled);
                }}
              />
              <h1 className="font-bold text-purple-600 text-nowrap peer-checked:text-gray-500">
                Choose a Color:{" "}
              </h1>
              <input
                value={input3}
              ...
              {colorErr && (
                <p className="absolute top-full text-red-500 text-xs mt-1 ml-5">
                  Please enter a new color or deselect the field.
                </p>
              )}
            </div>
            <button
            ...
            <button
              className="text-lg absolute left-0 bottom-0 bg-red-500 p-2 rounded-xl hover:bg-red-600 text-white transition-colors duration-175 ease-in-out font-black cursor-pointer m-5"
