  ...
  return (
    <div className="flex flex-col w-2/3 h-full place-content-evenly">
      ...
      <div className="flex flex-row justify-start items-center">
        <h1 className="text-center uppercase text-xs text-white font-black">
          Tags:
        </h1>
        {guide.tags !== undefined
          ? guide.tags.map((tag) => (
              <div
                key={tag.id}
                style={{ backgroundColor: tag.color }}
                className={` w-max min-w-10 text-center p-2 ml-2 rounded-2xl text-[0.65rem] font-bold text-white`}
              >
                {tag.name}
              </div>
            ))
...
function FilteredGuide({ guide }: { guide: StudyGuide_t }) {
  return (
    <div className="w-full flex flex-col justify-center items-center">
      <Link
        href={`/study/${guide.id}`}
        className="flex shrink-0 flex-row place-content-evenly border-2 rounded-xl w-3/5 h-50 m-3 bg-purple-600 text-black shadow-lg transition hover:shadow-xl hover:scale-[1.01]"
      >
        <Icon image={null} />
        <Content guide={guide} />
        <Progress />
